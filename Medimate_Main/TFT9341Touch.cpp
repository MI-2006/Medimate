...
void tft9341touch::begin()
{
	...
	if(_cs==10 && _dc==9 && _Tcs==7 && _Tirq==2)
		SPI.beginTransaction(SPISettings(SPI_FREQUENCY_UNO, MSBFIRST, SPI_MODE0));
	else
...
void tft9341touch::print (int32_t num)
{
	char str[33];
...
void tft9341touch::println (uint32_t num)
{
	print (num);
	_x = 0;
	_y += 8 * _fontSize +2;
...
void tft9341touch::print (uint32_t num)
{
	char str[33];
...
void tft9341touch::println (int32_t num)
{
	print (num);
	_x = 0;
	_y += 8 * _fontSize +2;
...
void tft9341touch::print (int32_t num, uint8_t radix)
{
	char str[33];
	itoa (num, str, radix);
	print (str);
...
void tft9341touch::println (int32_t num, uint8_t radix)
{
	print (num, radix);
	_x = 0;
	_y += 8 * _fontSize +2;
...
void tft9341touch::print (uint32_t num, uint8_t radix)
{
	char str[33];
...
void tft9341touch::println (uint32_t num, uint8_t radix)
{
	print (num, radix);
	_x = 0;
	_y += 8 * _fontSize +2;
...
void tft9341touch::print (int16_t num)
{
	char str[17];
...
void tft9341touch::println (int16_t num)
{
	print (num);
	_x = 0;
	_y += 8 * _fontSize +2;
...
void tft9341touch::print (int8_t num)
{
	char str[9];
...
void tft9341touch::println (int8_t num)
{
	print (num);
	_x = 0;
	_y += 8 * _fontSize +2;
...
void tft9341touch::print (uint8_t num)
{
	char str[9];
...
void tft9341touch::println (uint8_t num)
{
	print (num);
	_x = 0;
	_y += 8 * _fontSize +2;
...
bool tft9341touch::color_gamma(uint8_t gamma)
{
	Wire.begin();
	...
	for (byte i = 0; i < 3; i++) {
		...
		if (Wire.available())
			tft_gamma[i] = Wire.read();
	}

	if (tft_gamma[0] == 0x31 && tft_gamma[1] == 0xF3 && tft_gamma[2] == 0x8E) {
		return true;
	}
	else {
		while (true) {}
...
void tft9341touch::print (uint16_t x,uint16_t y,String txtMsg)
{
	setCursor(x,y);
	print (txtMsg);
}
...
void tft9341touch::printHebrew (char *str)
{
	int8_t i=0;
	while(str[i++]);
	i--;
...
void tft9341touch::printHebrewln (char *str)
{
	printHebrew (str);
	_x = 0;
	_y += 8 * _fontSize +2;
...
void tft9341touch::printHebrew (uint16_t x,uint16_t y,char *str,uint16_t fontSize, uint16_t fColor)
{
	setCursor(x,y);
	setTextSize(fontSize);
	setTextColor(fColor);
...
void tft9341touch::printHebrew (uint16_t x,uint16_t y,char *str,uint16_t fontSize, uint16_t fColor,uint16_t bColor)
{
	setCursor(x,y);
	setTextSize(fontSize);
	setTextColor(fColor,bColor);
...
uint16_t tft9341touch::ADS_Read_AD(uint16_t CMD){
	uint8_t data1, data2;
	uint16_t dx;
...
{	
	...
	while(1)
	{
		xr[0]=ADS_Read_AD(0xD0); //0xD0
		delayMicroseconds(5);
		xr[1]=ADS_Read_AD(0xD0);
		delayMicroseconds(5);
		xr[2]=ADS_Read_AD(0xD0);

		if((abs(xr[0]-xr[1])< 10) && (abs(xr[0]-xr[2])< 10)  ) break;
	}
...
uint16_t tft9341touch::getToucY()
{
	...
	while(1)
	{
		yr[0]=ADS_Read_AD(0x90); //0x90
		delayMicroseconds(5);
		yr[1]=ADS_Read_AD(0x90);
		delayMicroseconds(5);
		yr[2]=ADS_Read_AD(0x90);

		if((abs(yr[0]-yr[1])< 10) && (abs(yr[0]-yr[2])< 10)) break;
	}
	y= (yr[0]+yr[1]+yr[2])/3;
...
void tft9341touch::setTouch(uint16_t xs1, uint16_t xs2, uint16_t ys1, uint16_t ys2)
{
